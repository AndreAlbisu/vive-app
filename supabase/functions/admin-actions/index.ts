...
serve(async (req) => {
  ...
  switch (body.action) {
    ...
    case 'list_coach_applications': {
      ...
      const { data, error } = await admin
        .from('coaches')
        .select('id, profile_id, specialty, bio, price_per_session, nationality, application_video_url, estilo, guia, focos, created_at, verified, application_status, application_notes, application_reviewed_at, compromiso_derivar, respuesta_riesgo, pais_atencion, provincia_atencion, profiles!inner(name, email)')
