...
- [ ] 🚨 **BORRADO DE CUENTA DENTRO DE LA APP — bloquea la publicación en iOS.** Apple lo exige (guideline 5.1.1(v)) para toda app que permita crear cuenta. Verificado el 06/08/2026: **no existe**; el menú de perfil solo tiene "Cerrar sesión". No es un formulario que se completa, es una feature que hay que construir (probablemente edge function con service role, porque borrar el usuario de `auth.users` no se puede desde el cliente).
