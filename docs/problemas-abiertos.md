...
| **L58** | **Auditoría tanda 5 (23/09, Claude)**: la sesión (refresh token) se guarda en AsyncStorage, no en el llavero cifrado del sistema. Es lo estándar en Expo y el sandbox del teléfono la protege salvo en equipos rooteados o con extracción de backup. Pasarla a `expo-secure-store` pide un adaptador con partido en trozos (límite de 2048 bytes) y **desloguea a todos** al actualizar. | 🟡 Decisión de Andre. Confirmado el 23/09: **no hay usuarios, así que no desloguea a nadie** y el cambio sale gratis. Recomendado hacerlo antes de publicar. |
