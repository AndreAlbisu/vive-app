...
| **L44** | 🔒 **Auditoría de seguridad: primera tanda hecha, el resto pendiente.** `supabase db advisors --linked --type security` devolvió **86 hallazgos**. Se cerraron los peores (ver L45). **Quedan 56**, y los que siguen en prioridad son: **4 vistas `SECURITY DEFINER`** (nivel ERROR: `coach_credentials_public`, `coach_availability_status`, `coach_rebooking_stats`, `coach_trending_stats`), que saltean la RLS de las tablas que leen; **26 funciones sin `search_path` fijo**, que en una `security definer` es un vector de inyección; y el toggle de *leaked password protection* en el panel de Supabase. Los otros dos grupos son esperados: 17 funciones ejecutables por `authenticated` (las de la app, todas con su propio chequeo adentro) y 5 tablas con RLS y cero policies (el patrón correcto para las que solo toca el servidor). | 🔴 Abierto. Correr el advisor de nuevo después de cada tanda: sirve de marcador. | — |
