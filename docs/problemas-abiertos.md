...
| **L47** | 🔴 **Auditoría tanda 2 (23/09)**: se podía **confirmar una sesión sin pagarla** con los 33 profesionales sin Mercado Pago. Cerrado en base + app + sala (ver SCHEMA.md). | ✅ Cerrado y verificado contra producción. Pendiente menor: limpiar las dos policies que todavía permiten confirmar "sin intento de cobro". |
| **L48** | **Auditoría tanda 3 (23/09)**: cerrado — en la Sala se podían fabricar avisos de sistema con el sello de Vita. Ver SCHEMA.md. | ✅ Cerrado y verificado. Sin hallazgos en perfiles, almacenamiento, secretos, funciones definer ni tiempo real. |
| **L49** | **Auditoría tanda 4 (23/09)**: quitado el INSERT de `profiles` al cliente (podía nacer con `is_admin=true` si faltaba su fila). Sin hallazgos en alta, baja, bloqueos y el ingreso por código de la sala web. | ✅ Cerrado y verificado. |
| **L50** | **Auditoría tanda 5 (23/09)**: la sesión (refresh token) se guarda en AsyncStorage, no en el llavero cifrado del sistema. Es lo estándar en Expo y el sandbox del teléfono la protege salvo en equipos rooteados o con extracción de backup. Pasarla a `expo-secure-store` pide un adaptador con partido en trozos (límite de 2048 bytes) y **desloguea a todos** al actualizar. | 🟡 Decisión de Andre. Sin usuarios reales, ahora es el momento más barato para hacerlo. |
