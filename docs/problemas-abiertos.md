...
| **L6** | **Nombre que ve el comprador en Mercado Pago.** 🔴 **La descripción vieja de esta fila estaba equivocada**: decía que salía de la config de la cuenta de MP y no del código. Es al revés. En el split el `collector` es la cuenta del **coach**, así que MP saca el descriptor de ahí y hay uno distinto por profesional: no existe ninguna config central que tocar. Verificado contra el pago real `174555144528`, que en el resumen de la tarjeta dice **`MERPAGO*AUGUSTOUNSAIN`**, el nombre personal del profesional. | ⚠️ **Código hecho y deployado el 21/09**: `mp-create-payment` v51 manda `statement_descriptor: 'VITA'`. MP lo acepta y lo devuelve tal cual en una preferencia de marketplace (probado). **Falta confirmarlo en un pago de verdad**, porque el texto final lo normaliza MP. Se cierra con el pago de la sesión de prueba de L1. | Con L1 |
