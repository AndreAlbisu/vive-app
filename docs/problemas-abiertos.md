...
| **L5** | **Revisión del abogado** de Términos, Privacidad y Reembolsos (desde el 25/09 los textos están completos y `LEGAL_IS_DRAFT = false`, pero eso solo dice que no quedan corchetes: nadie de afuera los revisó), con A.12 (§10.3) y la duda de si declarar Cloudflare/unpkg obliga a re-pedir consentimiento — que hoy no tiene mecanismo. | 🔴 Abierto. | Andre |
...
| ID | Qué falta | Estado |
|---|---|---|
| **L8** | **Correr `scripts/limpiar-datos-de-prueba.sql`** (coaches falsos, 24 reseñas falsas, 8 recursos `[SEED]` con dos rickrolls). | 🔴 No corrido, y desde el 23/09 **sin nada que lo frene**: Andre confirmó que todo el contenido de la base es de prueba (L9, L10, L11 despejados). Antes de correrlo: completar la lista `conservar` (hoy solo `andre`) y decidir si `coach-prueba` se va con el resto (L13). |
| **L9** | **Qué hacer con las reservas que movieron plata**: USD 91 en PayPal reembolsados + 15 `aprobado`. | 🟢 **Aclarado por Andre el 23/09: no hay usuarios ni profesionales reales, todo fue prueba.** O sea que la conservación de 10 años de Privacidad §10 **no protege a nadie acá**: no hay titular de datos del otro lado. ⚠️ Lo que sí es real es **la plata que se movió** (pagos y reembolsos de prueba de Andre): eso vive en Mercado Pago y PayPal, que son el registro contable, no en `bookings`. Con eso, borrarlas es seguro. |
| **L10** | ~~Mirar una por una las 6 reservas confirmadas~~ | ✅ **Cerrado el 23/09**: Andre confirmó que **no hay reservas de personas reales**; las 6 son de prueba. Se van con L8. |
| **L11** | **Qué significa `verified = true`** | 🟢 **Despejado el 23/09**: los 34 que lo tienen son perfiles de prueba y se borran en L8, así que hoy no afirma nada sobre nadie. **La decisión vuelve a hacer falta con el primer profesional real**: qué se chequea antes de marcarlo (matrícula ya tiene su propio circuito). |
...
| **L59** | **Auditoría de seguridad, tanda 1 (23/09, Claude)**: hallazgo cerrado — `application_notes` (el motivo de rechazo) era legible por cualquiera con la anon key. Ver SCHEMA.md y `scripts/cerrar-columnas-postulacion.sql`. | ✅ Cerrado. Falta auditar `lib/` (booking y pagos) y `app/`/`hooks/`. |
| **L60** | 🔴 **Auditoría tanda 2 (23/09, Claude)**: se podía **confirmar una sesión sin pagarla** con los 33 profesionales sin Mercado Pago. Cerrado en base + app + sala (ver SCHEMA.md). | ✅ Cerrado y verificado contra producción. Pendiente menor: limpiar las dos policies que todavía permiten confirmar "sin intento de cobro". |
| **L61** | **Auditoría tanda 3 (23/09, Claude)**: cerrado — en la Sala se podían fabricar avisos de sistema con el sello de Vita. Ver SCHEMA.md. | ✅ Cerrado y verificado. Sin hallazgos en perfiles, almacenamiento, secretos, funciones definer ni tiempo real. |
| **L62** | **Auditoría tanda 4 (23/09, Claude)**: quitado el INSERT de `profiles` al cliente (podía nacer con `is_admin=true` si faltaba su fila). Sin hallazgos en alta, baja, bloqueos y el ingreso por código de la sala web. | ✅ Cerrado y verificado. |
| **L58** | ~~La sesión se guardaba en AsyncStorage y no en el llavero del sistema~~ | ✅ **Hecho el 23/09** (`lib/secureSessionStorage.ts`, 8 tests): en el teléfono la sesión vive en Keychain/Keystore, partida en pedazos (el llavero rechaza valores grandes), con `AFTER_FIRST_UNLOCK_THIS_DEVICE_ONLY` para que la renovación en segundo plano siga funcionando y para que no viaje a iCloud, migración de lo que hubiera en AsyncStorage y caída a AsyncStorage si el llavero falla. 🔴 **Y hace falta un cliente de desarrollo NUEVO para que sirva**: `expo-secure-store` es código nativo y este proyecto no usa Expo Go sino `expo-dev-client`, así que el build instalado no lo tiene. Mientras tanto la sesión **sigue en AsyncStorage** (el código lo detecta y cae ahí solo, sin romper nada). ⚠️ En iOS el llavero sobrevive a desinstalar la app: reinstalar deja la sesión abierta. |
| **L63** | **Auditoría tanda 6 (23/09, Claude)**: el push del chat mandaba los primeros 50 caracteres del mensaje (y el título del recurso recomendado). Eso se ve en la pantalla bloqueada y viaja a Expo/Apple/Google, y la Política §12 no declara contenido. | ✅ **Cerrado**: el aviso ya no lleva texto del mensaje. ✅ **Y los avisos de reserva también, el 23/09**: Andre eligió sacar los nombres y dejar fecha y hora, en los dos sentidos (el cliente ya no ve el nombre del profesional, y el profesional ya no ve el del cliente). Los mails sí los conservan: no se leen sin abrir el teléfono. |
...
| **L43** | 🟠 **El punto naranja de Mensajes no se apagaba nunca.** Tres defectos distintos, encontrados por un reporte de Andre: **(a)** el punto se encendía con `hasAnyUnread || hasBookingToday`, y ese segundo término **no se puede "leer"**: con una sesión confirmada hoy quedaba prendido todo el día hiciera lo que hiciera la persona. **(b)** `checkBookingToday` calculaba "hoy" con `toISOString()`, o sea en **UTC**: entre las 21:00 argentinas y la medianoche, una sesión de mañana ya contaba como de hoy. **(c)** un mensaje que llegaba **mientras el chat estaba abierto** no se marcaba leído, porque la marca se escribía solo al entrar. | ✅ **Arreglados los tres el 21/09.** El punto vuelve a significar una sola cosa (hay algo sin leer), "hoy" se calcula con `todayInAr()`, y la sala se marca leída **también al salir**. 📱 Falta verlo en el teléfono. | — |
| **L44** | 🔒 **Auditoría de seguridad: dos tandas hechas.** `supabase db advisors --linked --type security` arrancó en **86 hallazgos** y está en **30**. Lo cerrado: L45 (funciones invocables por cualquiera) y las **26 funciones sin `search_path` fijo**. ✅ **Las 4 vistas `SECURITY DEFINER` (nivel ERROR) se revisaron una por una y son FALSOS POSITIVOS**: exponen solo agregados por profesional (`coach_id` + contadores) o credenciales ya verificadas, y **probado desde afuera con la clave pública, las tablas que leen (`bookings`, `coach_credentials`) siguen devolviendo vacío**. No son una puerta trasera: son el mecanismo correcto para publicar datos derivados sin abrir la tabla privada. **No convertirlas a `security_invoker`: rompería el catálogo público.** | 🔴 Lo que queda: el toggle de *leaked password protection* (un clic en el panel de Supabase), `extension_in_public`, y las dos categorías esperadas (17 funciones ejecutables por `authenticated`, que son las de la app con su chequeo adentro, y 5 tablas con RLS y cero policies, que es el patrón correcto de lo que solo toca el servidor). Falta además la auditoría de CÓDIGO: `find-bugs` scopeada por carpeta, empezando por `supabase/functions/`. | — |
| **L45** | ~~🔴 **Quince funciones internas eran invocables por cualquiera** vía `/rest/v1/rpc/`, incluidas `expire_pending_bookings()` (cancela reservas y las pasa a reembolso) y `complete_confirmed_sessions()`.~~ | ✅ **Cerrado 21/09.** Revocado `execute` a `public` (el detalle clave: revocarle solo a `anon` y `authenticated` no hace nada). Verificado desde afuera: 401. El cron y los triggers siguen funcionando; `slots_libres` y `email_es_de_coach` siguen abiertas a propósito. | — |
| **L46** | 🔒 **Auditoría de código, tanda 1: `supabase/functions/`.** Seis hallazgos, ninguno crítico, **los seis arreglados el 22/09**: (1) el webhook de MP no comparaba el monto cobrado contra el esperado, a diferencia del de PayPal; (2) la firma del webhook de MP no validaba la frescura del `ts`, así que una notificación capturada se podía reenviar para siempre; (3) `mail-notificaciones` no chequeaba `esServiceRole`, la única de las siete de cron sin el candado; (4) en `weekly-reflection` las **claves** de `facts` entraban al prompt sin validar (inyección acotada al propio usuario); (5) `delete-account` calculaba "hoy" en UTC, así que entre las 21:00 y la medianoche una sesión de esa noche no contaba como futura; (6) `daily-diagnostico` sin ningún control de acceso (no estaba deployada). | ✅ Cerrados. **Falta la tanda 2: `lib/`**, y después las policies RLS del schema. 📌 No se leyó entero `admin-actions` (1116 líneas: se revisó el portón y las URLs firmadas) ni `_shared/booking-effects.ts`, `commission.ts`, `guarantee.ts`, `usdt.ts`. | — |
| **L47** | 🔴 **Cualquiera con una cuenta podía mandarle una notificación a cualquiera, con el texto que quisiera, y esas notificaciones SE MANDAN POR MAIL desde nuestro dominio.** La policy de INSERT solo pedía estar logueado, y los `profile_id` de los profesionales son públicos: alcanzaba una cuenta para mandarle a todos un mail que parece de Vita, con DKIM y SPF válidos. | ✅ **Cerrado 22/09**: solo se avisa sobre una reserva propia y a la otra parte de esa reserva. Probado con 6 casos. ⚠️ **Queda abierto un resto**: quien tiene una reserva con alguien puede mandarle texto libre por esa vía, igual que por el chat. Cerrarlo del todo es mover los 7 inserts del cliente a una función `security definer`. | — |
| **L48** | 🔴 **Un profesional podía darse a sí mismo la matrícula verificada**, insertando su credencial con `status = 'verificada'`. Probado con un coach real: `has_matricula` pasó a `true` y la credencial inventada salía en la ficha pública. Es el sello del que dependen el buscador, el quiz y la pregunta de enfoque. | ✅ **Cerrado 22/09.** `status` ya no es escribible desde el cliente. 📌 La trampa: revocar por columna no resta de un grant de TABLA, hay que revocar el de tabla y regrantear columna por columna. | — |
| **L49** | 🔴 **Se podía escribir en la conversación de dos desconocidos** (`messages` solo chequeaba `sender_id`) y **mudarle la sala a otro profesional** (`coach_id` escribible por el cliente). | ✅ **Cerrados 22/09**, los dos probados con rollback. ⚠️ Queda abierto `sender_type`: un participante puede mandarse un mensaje disfrazado de sistema. | — |
| **L50** | 🔴 **`resource_completions` nunca guardó una sola fila.** RLS activada y **cero policies**, así que el insert del cliente se denegaba siempre, y `lib/resourceCompletions.ts` hacía el `await` **sin mirar el error**. Verificado: `count(*) = 0` con la app en uso desde junio. La escriben ocho pantallas de prácticas y de ella se deriva el "hecho hoy" de los hábitos, así que **la racha, el progreso y la tarjeta de continuar una práctica a medias no funcionaron nunca**. | ✅ **Cerrado 22/09**: tres policies (ver, anotar y actualizar lo propio) y el error ahora se loguea. Probado: anotar lo propio ✅, anotarle a otro ❌, un tercero no ve nada ✅. 📌 **Tercer bug de la misma familia en dos días** (los otros: el recordatorio nocturno y la cancelación de competidores). Los tres eran un `await` sin `if (error)`. | — |
| **L51** | 📜 **Los cuatro desajustes entre los T&C y lo que la app hace.** | ✅ **Tres resueltos el 22/09.** (a) §8.3 ahora dice las dos escaleras de comisión, incluida la **25%/20%** del riel internacional. (b) §8.1 y §8.2 describen las dos modalidades: en pesos el profesional cobra directo por el split de MP; desde el exterior **Vita percibe por cuenta y orden y después liquida**, con un corchete explícito pidiéndole al abogado que confirme el encuadre de mandato de cobranza. (c) §9.5 **dejó de ser una promesa vacía: ahora se ejecuta sola** (ver L52). ⚠️ **(d) sigue abierto**: §8.4 habla de los comprobantes que Vita emite y el sistema **no emite ninguna factura**. Eso no se arregla redactando: es la conversación con el contador (`docs/fiscal-instrucciones.md` §2.1). | Andre + contador |
| **L52** | 🔴 **§9.5 prometía un reintegro automático por plantón que no ejecutaba nadie.** | ✅ **Construido el 22/09.** `_shared/asistencia.ts` (10 tests) decide el veredicto con los registros de conexión de Daily, que traen el `user_id` de cada participante y **son nuestros propios ids de perfil**. Lo aplica `session-attendance`, que ya tenía la evidencia en la mano: si el profesional no entró en 10 minutos, o se fue antes de que llegara el cliente, la reserva pasa a `cancelada` con `cancelled_by = 'coach'`, que es lo que hace que el trigger devuelva la plata. 📱 **Sin ver en producción todavía**: hay que confirmarlo con una sesión real a la que el profesional no entre. | — |
| **L53** | 💰 **Chequeo exhaustivo del modelo de dinero, verificado contra código y base** (`docs/mapa-del-dinero.md`, nuevo): qué se cobra, cómo, en manos de quién queda, qué hay que facturar y qué preguntarle al contador. | ✅ **Hecho el 22/09.** Lo verificado: el precio sale siempre de la base y las tres funciones de cobro chequean que la reserva sea tuya; **el split de MP está activo y no está en modo prueba** (comprobado por evidencia, la `application_fee` del pago con tarjeta fue el 15% exacto); **no hay promo activa**; y la comisión retenida y no devuelta en toda la historia es **$2,70 y USD 1,20**. 🔴 **Corrige `fiscal-instrucciones.md`**, que daba por confirmado un pago de $4.500 que la base dice reembolsado. ⚠️ **Deudas reales detectadas**: USD 4,80 sin liquidar a un profesional del riel USDT, y un reembolso de $1 trabado desde el 09/08. | Andre |
| **L54** | 🔴 **M16 tenía dos agujeros de acceso, encontrados por Andre preguntando "¿cómo propongo mover una sesión?".** (a) En Reservas, el "⋯" **desaparecía** en la próxima sesión dentro de las 24hs (lo reemplazaba "Preparar"): o sea que **justo cuando más falta hace mover una sesión, el único camino no existía** y no quedaba más que cancelar, que es lo que M16 vino a evitar. (b) Desde la **agenda** del profesional, que es donde mira su semana, tocar una sesión solo abría el chat. (c) Y después de proponer, el profesional **no veía nada**: ni qué propuso, ni que estaba esperando. | ✅ **Los tres cerrados el 22/09.** "Preparar" y el "⋯" ahora conviven; la agenda ofrece el mismo menú al tocar una sesión (sin "cancelar", que vive entero en Reservas y duplicarlo sería tener dos cancelaciones que se pueden separar); y Reservas muestra "Horarios que propusiste" como estado. ✅ **Retirar una propuesta, agregado el 23/09** (`scripts/add-retirar-propuestas.sql`): botón "Retirar los horarios" en Reservas. Se retira por reserva y no por opción, las filas quedan en `vencida` y **se le avisa al cliente**. Probado con rollback: el cliente no puede retirar una oferta ajena, retirar dos veces frena, y la sesión no se mueve. | — |
| **L55** | 🎯 **El código de referido se canjeaba desde el Perfil en cualquier momento antes de pagar.** Eso no medía referidos: quien llegaba solo podía pedirle un código a un amigo justo antes de pagar y llevarse el 10% igual, o sea **un descuento universal en la primera sesión**, que es la práctica que se decidió no copiarle a Selia. Y el momento natural, crear la cuenta, **no lo pedía en ningún lado**. | ✅ **Movido al registro el 23/09.** Campo opcional detrás de un link ("¿Te invitó alguien?"), el código se guarda y lo canjea `AuthContext` al aparecer la sesión, así cubre los tres caminos de alta (mail, Google, Apple) en un solo lugar. En la base, `canjear_codigo` ahora frena si la persona **ya reservó**, no solo si ya pagó. El Perfil conserva solo tu propio código para compartir. ⚠️ **Falta que el link haga algo**: el mensaje que se comparte ya lleva `?ref=<código>`, pero ni la landing ni la app lo leen todavía. Hasta entonces el código se tipea a mano. | — |
| **L56** | 🔗 **Los links de `vitaapp.com.ar` abren la web aunque la persona tenga la app.** Faltan los *universal links* (iOS) y *App Links* (Android): `app.json` tiene el esquema propio `viveapp://` pero **ningún dominio asociado**, y la web no tiene `.well-known/`. 📌 **Lo que más pesa NO es el referido**: hoy quien tiene la app y abre el link de un profesional (`/c/<slug>`) termina en el **checkout web**, identificándose de nuevo con un código por mail y pagando desde el navegador, con la app instalada en el mismo teléfono. | ⏸️ **Al publicar** (necesita los certificados de firma, que salen de EAS). Lo que hay que hacer: **(1)** `ios.associatedDomains: ["applinks:vitaapp.com.ar", "applinks:www.vitaapp.com.ar"]`; **(2)** `android.intentFilters` con `autoVerify`; **(3)** servir `web/.well-known/apple-app-site-association` con el appID `<TEAM_ID>.com.andrealbisu.viveapp` y `web/.well-known/assetlinks.json` con el paquete `com.andrealbisu.viveapp` y la huella SHA-256 de la firma; **(4)** ⚠️ en `vercel.json`, forzar `Content-Type: application/json` para el AASA (va **sin extensión**, y hay que confirmar que `cleanUrls` no lo rompa). ⚠️ **No resuelve el caso sin la app instalada**: ver L57. | — |
| **L57** | 📲 **Un código de referido no sobrevive a la instalación.** Si la persona no tiene la app, el link la manda a la tienda y el sistema operativo **no transporta nada** del otro lado. | 🟡 **Decisión tomada: se deja así.** Android tiene solución nativa y gratis (el `referrer` del link de Play, leído después de instalar); **iOS no tiene equivalente**, y la única salida sería un SDK de atribución (Branch, AppsFlyer) que identifica dispositivos por huella. **En una app de salud mental ese costo de privacidad no lo paga un 10% de descuento**: habría que declararlo en la Política y en las etiquetas de las tiendas. El camino que funciona en los dos sistemas por igual es el que ya está: **el código va escrito en el mensaje que se comparte y se tipea en el registro**. | — |
...
| **M16 bis** | **Y si ninguno de los horarios propuestos le sirve al cliente.** | ✅ **Hecho el 23/09.** Observación de Andre: quedaban dos salidas y las dos malas para quien quería seguir con esa persona. Ahora **contrapropone** un horario libre de la agenda real del profesional, y **él lo acepta** (segunda decisión de Andre: la agenda es suya). No le gasta al cliente la ficha de las 24hs, porque el cambio lo empezó el profesional. `scripts/add-contrapropuesta-horario.sql`, corrido y verificado. 📱 Falta verlo en el teléfono. |
...
| **D6** | **¿Se enteran ustedes cuando el piso dispara?** Hoy no hay logging ni alerta propia. Sin eso no pueden aprender ni hacer seguimiento al día siguiente — *"la única forma de cuidado sostenible sin guardia 24/7"*. | 🟢 **Mitad HECHA (10/09):** el disparo emite `piso_seguridad_mostrado` en `analytics_events` (`app/(tabs)/index.tsx`), no solo el tap a `/ayuda`. Ahora se PUEDE ver cuántas veces aparece y mirar el día siguiente. 🔴 **Sigue ABIERTA la otra mitad:** una alerta o seguimiento por-persona en tiempo real — eso crea un flujo de datos de crisis por-persona y necesita decisión de privacidad/consentimiento (se cruza con B4/D2). |
