...
⚠️ **Hay una deuda real con un profesional**: la sesión de USDT (USD 6) está
`completada` y **nunca se liquidó** (`paid_out_at` en null). Son USD 4,80 que
Vita le debe. Es la única, y es el ejemplo perfecto de cómo funciona el modelo B.
...
2. **¿Hay que facturar lo que ya pasó?** La respuesta práctica es "casi nada":
   **$2,70 y USD 1,20** de comisión retenida en toda la historia, todo de pruebas
   propias. Conviene llevar el número exacto y no la pregunta general.
...
- ⚠️ **Liquidarle los USD 4,80 al profesional del riel USDT**, o dejar registrado
  por qué no.
