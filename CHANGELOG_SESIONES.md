...
- **Hallazgo 2 — 🔴 bug real (gesto de back), en dos capas**:
  ...
  - Capa 2 (la que seguía dando error tras el primer fix, confirmado en dispositivo tras reiniciar Metro con `--clear` para descartar bundle viejo): **re-entrada**. El propio `router.replace` del handler dispara otro `beforeRemove`, y como el gesto **nunca marcaba `terminado`**, volvía a interceptar y llamaba a `abandonarAlta`/`deleteMyAccount` una segunda vez —ya sin sesión, sobre una cuenta borrada— → error y cuelgue. El botón "Cancelar" nunca lo tuvo porque `cancelar()` marca `terminado.current = true` antes de su `replace`. **Fix**: marcar `terminado.current = true` al inicio del intercept (antes del `replace`), + un `.catch` para que un abandono fallido no quede como unhandled rejection.
