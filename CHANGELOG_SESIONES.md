...
- 🔴 **Deployar `guarantee-claim`** (`npx supabase functions deploy guarantee-claim` desde tu Terminal, y confirmar `ACTIVE` con `functions list`). No sale desde este entorno: el CLI de Supabase no está logueado y se cuelga esperando login interactivo — mismo caso que `mp-create-payment` en la sesión 88, que terminó deployando Joaquín. **Hasta que se deploye, la garantía se opera 100% a mano.**
