...
- **Cerrado con privilegios por columna**, no con más policies: `revoke update … from authenticated` + `grant update (lista exacta)`. Las listas salieron de auditar todos los `.update()` del cliente — 5 columnas en `coaches`, 10 en `profiles`. Verificado antes de escribirlas que **nada en el cliente escribe `role` ni `verified`**, así que cerrarlas no rompe ningún flujo. Quedó como **regla crítica 19** de SCHEMA.md, con la pregunta que se desprende: ante cualquier policy "la fila es tuya", preguntarse qué columnas su dueño NO debería poder tocar.
...
Typecheck y lint limpios en los siete bloques de código. HTML de las 5 páginas validado (anidamiento correcto).
