...
- 🔴 **La hora con el contador → todo escrito en `docs/fiscal-instrucciones.md`** (nuevo), con las preguntas ordenadas, el contexto de cada una y qué se destraba con cada respuesta.
  - 🔴 **La primera de la lista apareció al final de la sesión y no es del riel internacional: Vita cobra comisiones desde el 09/08 y el sistema NO emite ninguna factura** — cero referencias a facturación, ARCA o AFIP en todo el código. Si no se vinieron emitiendo a mano por fuera de la app, hay comisiones cobradas sin comprobante. La cuota del Monotributo no reemplaza la obligación de facturar; son dos cosas distintas.
