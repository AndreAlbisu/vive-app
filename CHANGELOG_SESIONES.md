...
- **Conexiones — sistema de puertas + matching de coaches (backend, definido con Joaquín):** se resolvió el debate de "Todos los profesionales" (lista infinita al escalar + redundante con search3) con un **deck curado**. Se armó todo el backend, falta solo la UI del deck (pendiente de naming/visual de Joaquín). Piezas:
...
- **Conexiones — UI del deck:** enchufar las 10 puertas + `rankDeck` a `conexiones.tsx` (reemplazar `CHIPS`/lista plana), card de cierre cuando la puerta tiene 0 elegibles, y "Ver todos en [tema]" → `search3` pasando `door.subtemas.join(',')` (search3 ya filtra multi-subtema). Espera el OK de **naming/visual de Joaquín** (labels de puertas, color de la híbrida "Ansiedad y estrés"). Todo el backend ya está listo y probado.
