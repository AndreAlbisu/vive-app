...
- 🔴 **Lo que sigue sin probarse es el flujo live de punta a punta.** La configuración está verificada, pero eso no es lo mismo que un cobro real: falta ver si la verificación de firma se comporta igual en producción (duda que viene de la sesión 114) y si el checkout live funciona. **La prueba más barata son USD 20** — el CHECK impone ese mínimo, así que no se puede hacer con USD 1 como en Mercado Pago; el coach de prueba está en 30. Se recupera reembolsando, menos las comisiones, que en un reembolso no siempre vuelven enteras.
