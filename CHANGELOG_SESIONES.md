...
**Tocado:** `context/AuthContext.tsx`, `screens/LoginScreen.tsx`, `components/AuthModal.tsx`, `SCHEMA.md`. Nuevo: `scripts/add-social-profile-data.sql` (**corrido y verificado**).
...
- 🔴 **El provider de Apple está apagado en Supabase** (`/auth/v1/settings` devuelve `apple: false`). El código de `signInWithApple` está completo y correcto, pero del lado del servidor no está habilitado: hoy el login con Apple no funciona. Es **bloqueante para publicar en la App Store** — la guideline 4.8 lo exige justamente porque ofrecemos Google.
...
- **Verificar el alta con Apple**, que pasa por `signInWithIdToken` y no por el flujo web — el fix del `#` no la toca, pero tampoco se pudo probar con el provider apagado. Ojo con `credential.fullName`: Apple lo da **solo en la primera autorización** y hoy no se le pasa a Supabase, así que esas cuentas quedan con `name = 'Usuario'`.
