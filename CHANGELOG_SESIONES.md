...
- 🔴 **Y eso confirma lo peor de la auditoría: la clave de producción ES `vive_mvp_key_2026`, la que estuvo versionada en este repo.** Cualquiera que haya visto el código puede desofuscar todos los mensajes de la base. Rotarla es una decisión con costo: los mensajes viejos quedan ilegibles salvo que se los reescriba con una migración.
- 🔴 **Probar en dispositivo que se pueden mandar mensajes**, que es lo único que este cambio podría haber roto.
- ⚠️ **`mp-webhook` y `paypal-webhook` siguen con el `booking-effects` VIEJO** (el que falla abierto). No se tocaron ni se redeployaron: son el camino del dinero y no estaban en la lista. Deployarlas ahora sería seguro —el secret ya está y el valor es el mismo—, pero es decisión de Andre.
