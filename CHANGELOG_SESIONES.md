...
- 🔴 **Falta el caso que justifica todo el cambio: matar la app desde el multitasking mientras se paga**, y verificar que la reserva quede confirmada y el coach notificado igual. El camino feliz ya está probado; este es el que valida que la confirmación server-side de verdad corre sin el cliente. Si esto no anda, no hay ningún error visible — la reserva simplemente se queda en 'pendiente'.
- ⚠️ **No quedó registrado si abrió la app NATIVA de MP o si cayó al navegador in-app.** Son resultados distintos con la misma apariencia de éxito: el segundo significa que `openURL` sigue rechazando y que el objetivo del cambio (aprovechar la sesión y la biometría de la app de MP) no se está cumpliendo en iOS. Se distingue mirando la consola: si cayó al fallback hay un `Linking.openURL rechazó (...)`.
