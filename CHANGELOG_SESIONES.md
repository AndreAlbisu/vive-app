...
- 🔴 **Poner el panel en marcha**, en este orden: (1) correr `scripts/add-admin-flag.sql`; (2) darse de alta como admin con el `update … set is_admin = true` que está al pie del script — **desde el SQL editor**, porque desde la app tiene que fallar y eso es parte de la verificación; (3) deployar **`admin-actions`** y **re-deployar `guarantee-claim`** (cambió su chequeo de auth). Verificar las funciones sondeando el endpoint, como en la sesión anterior.
