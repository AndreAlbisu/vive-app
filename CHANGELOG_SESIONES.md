...
**Tocado:** `lib/moodStats.ts`, `app/(tabs)/index.tsx`, `app/diario.tsx`, `SCHEMA.md`. Nuevo: `components/CoachSuggestionCard.tsx`, `components/ResourceSuggestionCard.tsx`, `scripts/create-resource-recommendations.sql`.
...
- `resource_recommendations` (`scripts/create-resource-recommendations.sql`, documentada en `SCHEMA.md`): un evento por visualización con check-in real (`mood_id`, `mood_label`, `suggested_first`/`suggested_second`, `chosen` nullable con CHECK de que sea uno de los dos sugeridos o NULL). RLS 4-policies own-only, mismo patrón que `mood_entries`. **⚠️ No se corrió la migración — falta ejecutar el script en Supabase** antes de que el registro funcione; mientras tanto la tarjeta y la navegación andan igual, el insert/update solo falla silenciosamente (no rompe nada, pero no se guarda nada hasta correrlo).

**Pendiente para la próxima sesión:**
- **Correr `scripts/create-resource-recommendations.sql` en Supabase** (coordinado con Andre) — sin esto el registro de "Para vos ahora" no guarda nada.
- Probar en dispositivo real: que el umbral de 2 niveles de `CoachSuggestionCard` se sienta bien (ni muy sensible ni muy laxo), que el ruteo a sala/Mensajes/Conexiones ande según corresponda, que el dismiss por día funcione, y que el par de recursos de "Para vos ahora" se vea bien con el orden random. También confirmar visualmente que Home quedó bien acomodado sin el bloque de pinneados y que los 5 prompts de Diario se leen bien en pantalla.
