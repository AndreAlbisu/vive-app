...
- **Correr `scripts/add-account-deletion.sql` y deployar `delete-account`.** ⚠️ El script toca 3 FKs de tablas vivas; trae al final la query de verificación. **Hasta que se corra, la baja falla en el último paso** (deja la cuenta anonimizada pero existente en auth).
