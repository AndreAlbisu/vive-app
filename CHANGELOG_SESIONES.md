...
- **Pendiente sin resolver:** logout de coach lleva a la interfaz de usuario (`/(tabs)`) en vez del inicio bifurcado — `handleSignOut` en `CoachProfileScreen.tsx:154` hace `router.replace('/(tabs)')`. Diagnosticado, no corregido.

**Pendiente para la próxima sesión:**
- **Correr 3 migraciones en Supabase**: `scripts/add-resource-audio-storage.sql` (bucket audio), `scripts/add-pinned-resources.sql` (pin), `scripts/add-saved-resources-unique.sql` (dedup guardados)
- Corregir el logout de coach: `router.replace('/(tabs)')` debería ir al inicio bifurcado (¿`/onboarding-bifurcacion`?) — confirmar la ruta correcta
