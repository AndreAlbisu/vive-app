...
- 🔴 **Publicar el sitio — es lo único que queda y son pasos de Andre, no de código.** Todo el contenido está escrito y generado; falta (1) comprar el dominio en NIC.ar, (2) importar el repo en Vercel verificando que tome el `vercel.json` y no autodetecte Expo, (3) apuntar el DNS. Paso a paso en `docs/hosting.md`.
- Y **recién cuando el dominio resuelva**: completar la URL en T&C §9.4 + `npm run sync:legal`, cargar las URLs en App Store Connect y Google Play Console (privacidad + eliminación de cuenta), y probar en incógnito que la portada abra el botón de arrepentimiento sin sesión.
