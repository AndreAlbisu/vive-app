...
- **La redacción por IA queda construida, apagada y sin deployar.** Para encenderla: respuesta legal → `ANTHROPIC_API_KEY` en Supabase → deployar `weekly-reflection` → `EXPO_PUBLIC_AI_REFLECTION=true`. Los cuatro pasos son independientes y cualquiera que falte deja la app en el texto determinístico, sin romperse.
