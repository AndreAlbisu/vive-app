...
- 🟠 **Lag del switch "Guardar cómo venís" (reportado en la device review) — arreglado en dos pasadas.** El `Switch` era controlado (`value={consent.puede}`) y `responder` esperaba el round-trip a la edge function (server en São Paulo + cold-start, Joaquín en Australia = 1-2s) antes de mover el estado. 1ª pasada: **optimista** + `guardando` (deshabilitado mientras confirma) — mejor, pero jugar de un lado a otro seguía trabado porque cada toque bloqueaba el switch hasta que volvía la red. 2ª pasada: **el switch nunca se deshabilita** (siempre tappeable, optimista instantáneo) y las carreras se manejan con un **token last-write-wins** en `useConsent.responder` — un resultado de red de un toque viejo se descarta si hubo otro después; solo la falla del ÚLTIMO toque revierte (re-lee del server) + Alert. Seguro: el gate real (`getConsent`) lee del server aparte, el estado optimista solo afecta el display.
- ⚠️ Falta que Joaquín confirme el resto de la device review 157-163 (matrícula, "Sobre vos" early, ayuda).
- 🔎 **Pre-flight de la device review de matrícula (para Andre) — sin bugs.** Se leyó estáticamente todo el flujo que Andre no pudo probar desde la máquina: el trigger `sync_has_matricula` (el `IN (v_nuevo, v_viejo)` con NULL es seguro), las **4 superficies** de la insignia (`search3`, deck y resultados de `conexiones` vía el caché `coachesCache.ts` que sí trae `has_matricula`, y `BookingScreen_Confirm`), el filtro "Psicólogo" (`inferType` corta con `!hasMatricula → 'Coach'`, el bug viejo del texto libre está muerto), la card sage/sand del perfil (lee `coach_credentials_public`, ya filtrada por `verificada` — no afirma "verificada" sin serlo), y el aviso del admin (la edge function computa `coach_has_matricula`). **Todo correcto, ningún cambio de código.** 🔴 Lo único que queda para Joaquín en dispositivo (no verificable estático): el **trigger EN VIVO** — cargar una credencial Matrícula, aprobarla desde admin, y confirmar que `has_matricula` pasa sola a `true` + aparece la insignia y el perfil se pone verde. Hoy 0 coaches con matrícula, así que nada lo ejercita todavía.
- ✅ **`wellness_goal` obligatorio en el alta: YA estaba hecho** (verificado 04/09). `coach-recurso-nuevo.tsx` `validate()` ya lo exige (bloquea el guardado si falta). El 0/8 de la auditoría era solo data de seed que saltea el form, ya backfilleada. La pendiente estructural del plan estaba de más — corregido en `recomendacion-recursos.md §4bis`.
- 🟢 **Capa 1 arrancada: giro pasado→presente en `weeklyReflection.ts` (solo las 4 señales en pasado claro).** `la-voz-de-sofia.md` §1 decidió el 28/08 que la card deje de reportar la semana en pasado y hable en presente; el código nunca lo aplicó. Se giraron **`trend-up`, `trend-down`, `sessions`, `practices`** de afirmación-en-pasado a presente + pregunta (patrón §3.1/§3.2: referir lo concreto y preguntar por el ahora), respetando TODOS los guardrails. Ejemplos: "Esta semana venís mejor que las anteriores" → "Algo se está acomodando estos días. ¿Lo notás vos también?"; "Volviste X veces… Eso ya es una rutina" → "Volviste X veces… ¿Alguna te está sirviendo?". **`level` y `sustained-low` NO se tocaron** (están en presente pero con stance de reporte; son lo más sensible de la voz — se dejan para charlarlos con Andre). `tsc`/lint/**462 tests** limpios **sin tocar un solo test** (los tests de Andre son invariantes, no snapshots del copy). ⚠️ **Es código de la voz de Andre — el copy nuevo espera su visto bueno de voz.**
- 📝 **Reconciliados los planes con la devolución de Andre.** Se agregó una sección "§0 — Reconciliado (04/09)" arriba de `recomendacion-recursos.md`, `card-sobre-vos.md` y `plan-integral-acompanamiento.md`, que manda sobre la propuesta original: ranker con pesos → **filtro por intención** hasta 25-30 recursos; `user_consents` de Fase 2 → **prerrequisito** (C-184/20); `why` = enum cerrado; card vive en Recursos; **cerradas** las decisiones 1 y 4 de la card (no toda card lleva acción; el paquete va primero) y el "usuario nuevo" (era bug, ya arreglado con `early`). Orden acordado: consents → paquete → capa 1 (presente) → catálogo → ranker. Quedan 6 decisiones abiertas, la más importante **cómo crece el catálogo**.
