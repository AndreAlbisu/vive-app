...
- **`VITA` vs `VIVE`:** los documentos legales dicen VIVE y la app se llama VITA en ~10 pantallas (incluido el "de VITA" del checkbox, justo al lado del link que abre un documento que dice VIVE). Hay que decidir cuál es el nombre real y unificar antes de publicar.
- **Registrar la aceptación de los T&C**: hoy el checkbox es solo estado local, no queda constancia de qué versión aceptó cada usuario ni cuándo. Para que la cláusula anti-solicitación §10.2 sea oponible conviene guardar `accepted_terms_at` + versión del documento en `profiles` (requiere columna nueva, no se hizo por ser cambio de schema).
