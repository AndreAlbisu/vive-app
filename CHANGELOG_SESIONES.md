...
- **Correr `scripts/add-reports.sql` en Supabase** (sin eso, enviar un reporte falla — la hoja avisa, no finge éxito). Es solo frontend + tabla, no necesita redeploy de functions. Probar en Expo Go los dos puntos de entrada (chat en ambos sentidos, perfil del coach).
...
- Arrastrados de sesión 81: probar en dispositivo real todo lo de mood/recursos/diario; armar el dev build (sigue sin armarse); pre-booking (feature no construida); Sign in with Apple pausado a propósito. (Ya NO: filas rotas de `resource_reminders` + silencio de `ReminderBell.handleSave`, y evento de confirmación del coach — cerrados esta sesión.)
