...
- **Registrar la aceptación de los T&C — más chico de lo que parecía: `profiles.accepted_terms` (bool) YA EXISTE** (SCHEMA.md:18), no hace falta migración. El problema es que **nadie la escribe**: `RegisterScreen` le pasa `acceptedTerms` a `signUpWithEmail`, pero el parámetro está declarado y **no se usa** en el cuerpo de la función (`context/AuthContext.tsx:91`), así que el dato se pierde. Falta persistirlo (ojo con el timing: el row de `profiles` lo crea un trigger sobre `auth.users`, y con confirmación de mail puede no haber sesión inmediata). Para que §10.2 sea oponible convendría además guardar **cuándo** y **qué versión** aceptó — eso sí serían columnas nuevas.
