...
**Tocado:** `supabase/functions/mp-create-payment/index.ts` (**PENDIENTE DE DEPLOY**). Nuevo `scripts/cleanup-loose-test-payments.sql`.
...
- **⚠️ NO DEPLOYADO — falta correr `npx supabase functions deploy mp-create-payment` (o pegar el archivo en el dashboard).** Igual que con la limpieza de pagos, este entorno no tiene el CLI de Supabase logueado (`supabase functions deploy` y `supabase projects list` se cuelgan esperando un login interactivo que no puede completar). Hasta que se deploye, prod sigue contando las 16 reservas fantasma para la comisión.
...
- **Deployar `mp-create-payment`** con el fix del contador — sin esto, el cambio de esta sesión no tiene efecto en prod.
