...
**Tocado:** `screens/CoachHomeScreen.tsx`, `lib/coachDeckRanking.ts` (reescrito), `lib/coachesCache.ts`, `app/_layout.tsx`, `SCHEMA.md`. Nuevos: `lib/coachVisibility.ts`, `screens/CoachVisibilityScreen.tsx`, `app/coach-visibilidad.tsx`, `scripts/harden-reviews-insert.sql` (CORRIDO por Andre), `scripts/check-deck-pools.ts`.
...
- `CachedCoach.hasSlotThisWeek` nuevo, poblado en `_doFetch` con una cuarta query en paralelo contra `coach_availability_status`. (`app/(tabs)/conexiones.tsx` sigue haciendo su propio fetch de disponibilidad para el deck visible — quedó redundante, no se tocó para no ampliar el blast radius.)
