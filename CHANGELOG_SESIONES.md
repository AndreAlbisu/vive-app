...
- 🔴 **Correr `scripts/add-application-status-and-audit.sql`.** Nada de esta sesión funciona sin eso, y **el orden importa**: entre correr el SQL y tener la app nueva en el celular, el alta de coaches falla con 42501 porque la versión vieja manda `verified` en el INSERT. Si hay postulaciones en curso, deployá primero. Las 6 queries de verificación están al pie del script — **de a una**, que el SQL editor muestra solo la última.
- 🔴 **Deployar `admin-actions` (v1 → v2) y `guarantee-claim` (v3 → v4).** El CLI tenía sesión en la sesión anterior; vale re-chequear antes de asumir que no.
