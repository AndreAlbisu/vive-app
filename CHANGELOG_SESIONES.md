...
- 🚨 **BORRADO DE CUENTA DENTRO DE LA APP — bloquea la publicación en iOS.** Apple lo exige (guideline 5.1.1(v)) para toda app que permita crear cuenta. Verificado: **no existe**, el menú de perfil solo tiene "Cerrar sesión". No es completar un formulario, es construir la feature — probablemente edge function con service role, porque borrar de `auth.users` no se puede desde el cliente. **Es el bloqueador de publicación más grande que apareció hoy.**
