...
- **Correr `scripts/add-reports.sql` y `scripts/add-session-notes.sql` en Supabase** (sin correrlos, reportar / guardar-leer notas falla — las hojas avisan, no fingen éxito). Solo frontend + tablas, no necesitan redeploy de functions. Probar en Expo Go: reportes (chat ambos sentidos + perfil del coach) y notas de sesión (coach escribe privada+compartida; usuario ve la compartida).
