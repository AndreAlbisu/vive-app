...
- **Forma de `content` por `type`** (convención de frontend, no hay CHECK que la valide en la base — decidida en `screens/ProposeResourceScreen.tsx`, 04/07/2026):
  - `audio`: `{ url: string }` — link externo (Drive/YouTube/Spotify/archivo directo), no se sube el audio a Storage
  - `guia_pasos`: `{ steps: { title: string; body: string }[] }`
  - `lectura_breve`: `{ body: string; source?: string }`
...
- **Segunda sección agregada el mismo día**: `app/(tabs)/recursos.tsx` (biblioteca general que ve cualquier usuario) suma "Recursos de nuestros coaches" — scroll horizontal con lo publicado por cualquier coach (`attributed_to_coach_id is not null`), tarjeta con tipo/duración/nombre del coach, tap navega al perfil de ese coach (`/profesional?profileId=...`) en vez de duplicar el render de contenido que ya existe en `ProfesionalScreen.tsx`. Sección nueva y separada — no se tocó `TOOL_GROUPS` ni el mock `CoachSection`/`COACH_RESOURCES` que ya existía en ese archivo (ese es "recomendaciones de tu coach personal" sobre las tools de VITA, un concepto distinto que no se tocó). Solo visible con filtro "Todos", no bajo "Guardados" (no usa `saved_resources`).
